...
const servers = [
  ...
  },
  {
    ...
    lastHeartbeatAt: new Date(Date.now() - 12000),
    playerCount: 43,
    maxPlayers: 100,
  },
  {
    ...
    lastHeartbeatAt: new Date(Date.now() - 3000),
    playerCount: 62,
    maxPlayers: 150,
  },
  {
  ...
  },
  {
    ...
    lastHeartbeatAt: new Date(Date.now() - 7000),
    playerCount: 31,
    maxPlayers: 80,
  },
];
...
const alerts = [
  {
    ...
    status: "open",
    title: "서버 응답 없음",
    description: "5분 이상 heartbeat가 수신되지 않습니다. 즉각적인 확인이 필요합니다.",
  ...
  },
  {
    ...
    resolvedAt: new Date(Date.now() - 85800000),
  },
  {
    id: "alert-9",
    serverId: "us-1",
  ...
  },
  {
    ...
    resolvedAt: new Date(Date.now() - 172700000),
  },
];

function buildMetrics(server, range) {
  const config = {
    "1h": { count: 12, intervalMinutes: 5 },
    "6h": { count: 36, intervalMinutes: 10 },
    "24h": { count: 24, intervalMinutes: 60 },
  }[range];
  ...
  return Array.from({ length: config.count }, (_, i) => {
    const timestamp = new Date(
      now - (config.count - 1 - i) * config.intervalMinutes * 60 * 1000
    );

    if (isDown) {
      return {
        id: `${server.id}-${range}-${i}`,
        serverId: server.id,
        timestamp,
        cpuUsage: 0,
        memoryUsage: 0,
    ...
    return {
      ...
      serverId: server.id,
      timestamp,
      cpuUsage: Math.min(100, Math.max(0, baseCpu + drift * 2)),
      memoryUsage: Math.min(100, Math.max(0, baseMem + drift * 2)),
      rttMs: Math.max(1, baseRtt + drift * 4),
      errorRate: Math.min(1, Math.max(0, baseError + drift * 0.004)),
      playerCount: Math.max(0, server.playerCount + drift * 2),
    };
  });
}
...
async function seedRolesAndAdmin() {
  const userRole = await prisma.role.upsert({
    where: { name: "user" },
    update: {},
    create: { id: "role-user", name: "user", description: "일반 사용자" },
  });

  const adminRole = await prisma.role.upsert({
    where: { name: "admin" },
    update: {},
    create: { id: "role-admin", name: "admin", description: "관리자" },
  });
...
async function main() {
  ...
  const metrics = servers.flatMap((server) => [
    ...buildMetrics(server, "1h"),
    ...buildMetrics(server, "6h"),
    ...buildMetrics(server, "24h"),
  ]);

  await prisma.metricPoint.createMany({ data: metrics });
}
